...
def get_local_file_as_data_uri(file_name):
    """Lê um arquivo local na pasta atual e converte para Data URI."""
...
def file_to_data_uri(uploaded_file, file_name, fallback_mime="application/octet-stream"):
    """Prioriza o upload; se não houver, tenta o arquivo local."""
    ...
    class Game {{
        ...
        update(now) {{
            ...
            if (this.health <= 0) {{ 
                this.gameOver = true; 
                this.running = false; 
                if (audio) audio.pause(); // PARAR MÚSICA NO GAME OVER
                statusTxt.textContent = "GAME OVER!"; 
            }}
            
            if (ct > (audio?.duration || 120) + 1) {{ 
                this.finished = true; 
                this.running = false; 
        ...
        handle(key) {{
            ...
            this.flash[l] = 0.15; 
            this.pose = ["left", "down", "up", "right"][l]; 
            this.poseT = 0.25;

            if (d < 0.08) {{ 
                b.hit = true; this.score += 300; this.combo++; this.feedback = "PERFECT"; this.fbT = 0.6; this.health = Math.min(100, this.health+5); 
            }} else {{ 
                b.hit = true; this.score += 100; this.combo++; this.feedback = "GOOD"; this.fbT = 0.5; this.health = Math.min(100, this.health+2); 
            }}
        }}
        ...
        draw() {{
            ...
            if (img) {{ 
                const r = 350/img.height; 
                ctx.drawImage(img, 630-img.width*r/2, {HEIGHT}-80-350, img.width*r, 350); 
            }} else {{
                // Fallback Stickman
                ctx.strokeStyle = "white"; ctx.lineWidth = 5; ctx.beginPath();
                ctx.arc(630, {HEIGHT}-300, 30, 0, Math.PI*2); ctx.moveTo(630, {HEIGHT}-270); ctx.lineTo(630, {HEIGHT}-150); ctx.stroke();
            }}
            ...
            if (!this.running && !this.gameOver && !this.finished) {{
                this.drawOverlay("PRONTO?", "white", "Clique em JOGAR ou aperte ENTER");
            }}
        }}
